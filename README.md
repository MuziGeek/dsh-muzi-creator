...
插件通过 DSH 官方主题服务把会话区、输入区、工具卡片、设置、菜单、弹层和详情区统一到 Animal Island 的暖色浅色/深色视觉，同时保留 DSH 原始组件和交互。兼容皮肤固定适配 Desktop 2.0.4，不接管会话状态、审批、Composer 或设置数据；Windows 标题栏和系统对话框仍使用原生样式。
