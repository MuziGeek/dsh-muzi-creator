...
在「设置 → 插件 → 个人内容工作台 → 项目来源」选择「GitHub 仓库」后输入公开仓库链接（`https://github.com/owner/repository`、带 `.git` 后缀的链接或 `owner/repository`）、GitHub 用户名或用户主页链接（例如 `https://github.com/MuziGeek`），点击「读取仓库」，选择仓库及分支后添加项目。添加成功后项目来源切换到 GitHub；选择「本地目录」时仅显示目录设置，选择「GitHub 仓库」时仅显示仓库链接及相关配置；切换不会清空未保存的目录或仓库输入。本地目录在同一区域选择，通过设置页底部「保存」生效。来源切换及 GitHub 操作即时生效，不受「放弃」影响。项目侧栏保留列表、搜索与刷新，全局设置入口位于侧栏底部。
