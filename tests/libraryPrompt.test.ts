...
describe("libraryConventionText", () => {
  ...
  it("names the enabled publish platforms", () => {
    const text = libraryConventionText("/Movies/视频项目", "/.dsh-mz-creator", undefined, ["douyin", "wechat"]);
    expect(text).toContain("当前启用平台：抖音、视频号");
    expect(libraryConventionText("/Movies/视频项目", "/.dsh-mz-creator", undefined, []))
      .toContain("当前没有启用发布平台");
  });

  it("appends the configured script rules", () => {
    const text = libraryConventionText("/Movies/视频项目", "/.dsh-mz-creator", " 口语化，少用术语。 ");
    expect(text).toContain("当前脚本规则（人设）：");
    expect(text).toContain("口语化，少用术语。");
    const without = libraryConventionText("/Movies/视频项目", "/.dsh-mz-creator", "   ");
