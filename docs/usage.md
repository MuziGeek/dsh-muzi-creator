...
第一次用先让 AI 检查环境，再确认影片目录。字幕、封面、Screen Studio、Chrome 与平台验收都是可选能力，缺少时不会影响片库和脚本管理。字幕和封面没有 Key 时，对应按钮会提示去设置里填写。有成片后，检查器里可以生成字幕稿并打开预览，确认后再烧录；封面也可以在检查器里生成，不用再走对话。
