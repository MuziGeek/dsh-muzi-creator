...
export function DailyHotInspector({ t, closeDetails }: DailyHotInspectorProps) {
  ...
  return (
    ...
    >
      ...
      <div className="dailyHotInspectorScroll">
        ...
        {item.summary !== null && (
          <section className="dailyHotDetailSection">
        ...
        {item.sourceNames.length > 0 && (
          <section className="dailyHotDetailSection dailyHotSourceNames">
            <h2>{t("hot.sources")}</h2>
            <ul>{item.sourceNames.map((name) => <li key={name}>{name}</li>)}</ul>
          </section>
        )}
