...
“灵感”入口用于一次性主题研究和每日趋势监测。用户填写主题、目标、关键问题、来源语言、域名边界和研究深度后，可以立即启动研究或保存为待办；调研过程保持在灵感页面，绑定的可见 DSH Agent 会话只允许公开网页搜索、网页读取和只读知识检索。

每次运行必须通过结构化报告工具提交摘要、带来源的发现、分歧、创作角度、下一步和来源元数据。Host 将报告原子写入 `00-inbox/inspirations`，保留运行状态、来源校验和 SHA-256；来源不足时显示部分报告，失败、停止和中断不会自动重试。每日任务默认在 `Asia/Shanghai` 09:00 执行，首次启用需要明确授权，修改研究规格会自动暂停并要求再次授权。
...
- **灵感研究台账**：通过受限可见 Agent 进行主题研究和每日趋势监测，保存带来源的报告并在确认后生成内容方向提案。
...
- 灵感专用 Agent 拒绝文件、Shell、子进程、项目修改、Atlas 写入、内容创建、发布、消息发送和其他外部动作；每日任务只有在用户明确授权后才运行。
