...
左侧将“会话 / 内容 / 知识 / 项目”纵向排列。知识区域只展示 `wiki/topics` 主题页面，搜索也仅覆盖主题。顶部“预览”会在只读浮层中显示实时统计与 3D 主题中心知识星图；星图支持滚轮缩放、旋转视角和拖动节点，只使用正式 Wiki 中可唯一解析的显式 `[[Wiki 链接]]`，不会运行 llm-wiki 的离线图谱写入流程，也不会持久化节点位置。内容目录可直接新建，知识新增入口会切换到会话并交由标准 llm-wiki 流程写入。
...
详情面板读取活动任务和 `archive/YYYY-MM`，展示优先级、负责人、父子关系、时间、分支、相关文件和验证材料。只有归档任务同时满足 `status: completed`、存在 `completedAt` 且包含有意义的验证材料，才标记为“已验证完成”；其他归档任务明确显示“证据不足”。目录监听、防抖 revision 和窗口聚焦刷新会同步磁盘变化。
