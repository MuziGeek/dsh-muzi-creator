...
describe("DSH Desktop 2.0.4 host skin compatibility", () => {
  ...
  it("matches stable host fixtures while excluding the plugin root and portal", () => {
    ...
    expect(document.querySelector("#session-search > svg")).not.toBeNull();
    expect(document.querySelector("#session-view > svg")).not.toBeNull();
    expect(document.querySelector("#session-add > svg")).not.toBeNull();
  ...
  it("contains only scoped non-destructive rules and the responsive safety net", async () => {
    ...
    expect(css).not.toContain("--muzi-session-search-label");
    expect(css).not.toContain("--muzi-session-view-label");
    expect(css).not.toContain("--muzi-session-add-label");
    expect(css).toMatch(/button > svg\s*\{[\s\S]*?inline-size:\s*18px;[\s\S]*?block-size:\s*18px;/);
