...
const iconProvenanceFiles=new Set(['src/client/appearance/assets/icons/manifest.json','docs/iconography.md']);
...
const candidates=[...(await Promise.all(['src','tests','scripts','docs','skins'].map(dir=>files(join(root,dir))))).flat(),...['README.md','PRODUCT.md','DESIGN.md','package.json','tsdown.config.ts'].map(file=>join(root,file))];
const failures=[];
for(const file of candidates){
 ...
 if(name==='src/client/appearance/patches.css')text=text.replace(/url\("data:image\/png;base64,[A-Za-z0-9+/=]+"\)/g,'VERIFIED_ICON_BYTES');
