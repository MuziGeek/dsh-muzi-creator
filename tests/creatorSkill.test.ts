...
describe("creator-workbench skill", () => {
  it("registers a model-visible onboarding workflow", () => {
    ...
    expect(CREATOR_WORKBENCH_SKILL.content).toContain("总结成为母内容");
    expect(CREATOR_WORKBENCH_SKILL.content).toContain("整理为脚本");
    expect(CREATOR_WORKBENCH_SKILL.content).toContain("atlasLocator");
    expect(CREATOR_WORKBENCH_SKILL.content).toContain("目标已有非空内容时");
